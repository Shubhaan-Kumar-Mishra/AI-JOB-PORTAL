...
export const JobDetailsPage: React.FC = () => {
  ...
  // Try to use job object passed via location state if available, or fetch from API
    ...
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center bg-slate-950">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm font-medium text-slate-400">Fetching job details...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="glass-panel p-8 rounded-2xl border border-slate-800 space-y-8">
        ...
        <div className="border-b border-slate-800 pb-6 flex flex-col md:flex-row md:items-start justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row md:flex-col items-stretch gap-3 shrink-0">
            ...
            <button
              disabled
              title="Save Job will be enabled in Stage 4"
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            <Briefcase className="w-5 h-5 text-brand-400" /> Full Description & Requirements
          </h3>
        ...
        <div className="p-5 rounded-xl bg-brand-500/10 border border-brand-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <h4 className="text-xs font-bold text-brand-300 uppercase tracking-wider flex items-center gap-1.5">
              <Sparkles className="w-4 h-4 text-brand-400" /> Direct Adzuna Redirect
            </h4>
            <p className="text-xs text-slate-400">
              Clicking apply will redirect you directly to the original job listing page provided by Adzuna.
            </p>
          </div>
          <a
            href={job.url}
        ...
        {/* Mandatory Adzuna Attribution Banner */}
        <div className="pt-6 border-t border-slate-900 text-center text-xs text-slate-500 flex items-center justify-center gap-2">
          <span>Job listing sourced via</span>
          <a
